public class Archer extends Unit {
    public Archer(String name, int health, int attack_force, int attack_range, int shield, int moving_range, double available_moving_range, int cost, char prev_place, char place) {
    ...
    @Override public double fine_for_obstacles(int positionx, int positiony, Field game_field) { //штрафы за препятствия
        switch ((game_field.getField())[positionx][positiony]) {
            ...
            case ('#'): {// swamp
                this.setAvailable_moving_range(this.getAvailable_moving_range() - 1.8);
                break;
            }
            case ('@'): {// hill
                this.setAvailable_moving_range(this.getAvailable_moving_range() - 2.2);
                break;
            }
            case ('*'): { //usual
                this.setAvailable_moving_range(this.getAvailable_moving_range() - 1);
